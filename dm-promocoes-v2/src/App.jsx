...
function AbaCupons() {
  const [cupons, setCupons] = useState([
    { id:"CUP-001", codigo:"BEMVINDO15", tipoDesconto:"PERCENT", valor:"15", pedidoMin:"40", aplicarPara:"TODAS",    curvas:["A","B","C"], lojaEspecifica:"",        quemBanca:"50_50",  recorrencia:"SEMANAL", dias:["TER","QUI","SEX"], dataInicio:"2026-07-01", dataFim:"2026-07-31", status:"ATIVO",    obs:"Cupom de boas-vindas" },
    { id:"CUP-002", codigo:"FRETE5",     tipoDesconto:"FIXO",    valor:"5",  pedidoMin:"30", aplicarPara:"LOJA_ESP", curvas:[],           lojaEspecifica:"Burger House", quemBanca:"FRANQUIA", recorrencia:"SEMANAL", dias:["SEX","SAB"],       dataInicio:"2026-06-15", dataFim:"2026-07-15", status:"ATIVO",    obs:"" },
    { id:"CUP-003", codigo:"PROMO10",    tipoDesconto:"PERCENT", valor:"10", pedidoMin:"50", aplicarPara:"CURVA",    curvas:["A"],         lojaEspecifica:"",        quemBanca:"50_50",  recorrencia:"MENSAL",  dias:["SEG","TER","QUA","QUI","SEX","SAB","DOM"], dataInicio:"2026-05-01", dataFim:"2026-05-31", status:"EXPIRADO", obs:"" },
  ]);
  ...
  const salvar = (form, modoOverride) => {
    if (modoOverride==="editar") { setModalCupom({c:form, modo:"editar"}); return; }
    setCupons(l => { const existe=l.find(x=>x.id===form.id); return existe?l.map(x=>x.id===form.id?form:x):[form,...l]; });
    setModalCupom(null);
  };
  const excluir = (id) => { if(!window.confirm("Excluir este cupom?"))return; setCupons(l=>l.filter(x=>x.id!==id)); setModalCupom(null); };
...
export default function App() {
  ...
  const handleSave = (form, modoOverride) => {
    if (modoOverride === "editar") { setModal({ p: form, modo:"editar" }); return; }
    setLista(l => {
      const existe = l.find(x => x.id === form.id);
      return existe ? l.map(x => x.id === form.id ? form : x) : [form, ...l];
    });
    setModal(null);
  };
